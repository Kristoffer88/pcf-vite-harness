...
export interface DataverseErrorInfo {
  ...
  errorType: 'network' | 'auth' | 'not_found' | 'bad_request' | 'server_error' | 'rate_limit' | 'unknown';
  isRetryable: boolean;
  ...
  switch (status) {
    ...
    case 404:
      if (errorCode === '0x80040217') {
        ...
        suggestedActions.push('Use plural form for entity collections (e.g., "contacts" not "contact")');
      }
      break;
...
export async function testErrorAnalyzer() {
  ...
  const mockResponse = new Response(
    JSON.stringify({
      error: {
        code: '0x80060888',
        message: "Could not find a property named 'invalidfield' on type 'Microsoft.Dynamics.CRM.contact'."
      }
    }),
    {
      status: 400,
