...
import type { PCFContextUpdate, WebApiRequest } from '../utils'
...
export const PCFDevtoolsProvider: React.FC<PCFDevtoolsProviderProps> = ({
  ...
  const [pcfComponentRef, setPCFComponentRef] =
    useState<React.RefObject<ComponentFramework.StandardControl<any, any> | null>>()
  ...
  // Dataset Discovery State
  const [datasetDiscovery, setDatasetDiscovery] = useState<DatasetDiscoveryState>(
    createDatasetDiscoveryState()
  )
  ...
  const updateWebApiRequest = useCallback((id: string, updates: Partial<WebApiRequest>) => {
    setWebApiRequests(prev => prev.map(req => (req.id === id ? { ...req, ...updates } : req)))
  }, [])
  ...
  const discoverPCFControls = useCallback(async (manifest: PCFManifest) => {
    try {
      ...
      setDatasetDiscovery(prev =>
        updateDatasetDiscoveryState(prev, {
          isDiscovering: false,
          manifest,
          discoveredForms,
          controlsFound,
      ...
      console.log(
        `🔍 PCF Discovery complete: ${discoveredForms.length} forms, ${controlsFound.length} controls`
      )
    } catch (error) {
  ...
  const enhanceDatasets = useCallback(
    async (manifest: PCFManifest) => {
      if (!currentContext) {
      ...
      try {
        const result = await enhanceDatasetContext(currentContext, manifest, currentContext.webAPI)
  ...
  return <PCFDevtoolsContext.Provider value={contextValue}>{children}</PCFDevtoolsContext.Provider>
}
